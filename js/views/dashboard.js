// state.pasSel guarda el ID del PAS, no el nombre: hay nombres repetidos en la base, y filtrar por
// nombre sumaba en un solo filtro la producción de dos PAS homónimos (mismo motivo por el que el
// ranking de más abajo ya agrupa por _pas_id).
const filteredProduction = () => D.production.filter(x => (state.org === 'TODOS' || x.ORGANIZADOR === state.org) && (state.pasSel === 'TODOS' || String(x._pas_id) === String(state.pasSel)));
...
function dashboard() {
  ...
  // Mismo criterio que el ranking (ver comentario de arriba): estos dos KPIs contaban el catálogo
  // completo aunque hubiera un filtro de organizador/PAS aplicado, mientras "Pólizas cargadas" y
  // "PAS en producción" sí lo respetaban — dos números al lado sin nada que avisara que uno estaba
  // filtrado y el otro no. `comp` ya sale de las filas filtradas (ver arriba).
  const companiasFiltradas = Object.keys(comp).length;
  const organizacionesFiltradas = new Set(rows.map(x => x.ORGANIZADOR).filter(Boolean)).size;
  ...
  // Se arma por id (no por nombre): dos PAS homónimos en el mismo organizador tienen que quedar
  // como dos opciones separadas, cada una filtrando solo lo suyo.
  const exigeOrg = state.org !== 'TODOS';
  const pasPorId = {};
  if (exigeOrg) D.production.filter(x => x.ORGANIZADOR === state.org).forEach(x => { pasPorId[x._pas_id] = x.PAS; });
  const pasDisponibles = Object.entries(pasPorId).map(([id, nombre]) => ({ id, nombre })).sort((a, b) => a.nombre.localeCompare(b.nombre, 'es'));
